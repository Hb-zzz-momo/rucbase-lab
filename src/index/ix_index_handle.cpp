...
void IxNodeHandle::insert_pairs(int pos, const char *key, const Rid *rid, int n) {
    ...
    if (old_size > pos) {
        memmove(get_key(pos + n), get_key(pos), (old_size - pos) * file_hdr->col_tot_len_);
...
void IxNodeHandle::erase_pair(int pos) {
    ...
    int old_size = get_size();
    if (pos + 1 < old_size) {
        memmove(get_key(pos), get_key(pos + 1), (old_size - pos - 1) * file_hdr->col_tot_len_);
...
void IxIndexHandle::insert_into_parent(IxNodeHandle *old_node, const char *key, IxNodeHandle *new_node,
    ...
    int old_index = parent->find_child(old_node);
    parent->insert_pair(old_index + 1, key, Rid{new_node->get_page_no(), -1});
...
page_id_t IxIndexHandle::insert_entry(const char *key, const Rid &value, Transaction *transaction) {
    ...
    } else if (leaf->lower_bound(key) == 0) {
...
Iid IxIndexHandle::lower_bound(const char *key) {
    auto leaf_pair = find_leaf_page(key, Operation::FIND, nullptr);
    IxNodeHandle *leaf = leaf_pair.first;
    if (leaf == nullptr) {
        return leaf_end();
    }
    ...
    page_id_t page_no = leaf->get_page_no();
    while (slot == leaf->get_size() && page_no != file_hdr_->last_leaf_) {
        page_id_t next = leaf->get_next_leaf();
        buffer_pool_manager_->unpin_page(leaf->get_page_id(), false);
        leaf = fetch_node(next);
        page_no = leaf->get_page_no();
        slot = 0;
    }
    Iid iid{page_no, slot};
    buffer_pool_manager_->unpin_page(leaf->get_page_id(), false);
    return iid;
}
